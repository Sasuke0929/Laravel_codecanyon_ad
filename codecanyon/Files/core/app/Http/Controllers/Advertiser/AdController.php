...
class AdController extends Controller
{
    ...
    {
        $advertises = CreateAd::with('advertiser')->whereAdvertiserId(Auth()->guard('advertiser')->id())->latest()->paginate(15, ['id', 'advertiser_id', 'ad_name', 'ad_title', 'ad_type', 'budget' , 'status', 'resolution', 'image']);
    ...
    public function store(Request $request)
    {
        ...
        if($request->budget > auth()->guard('advertiser')->user()->balance){
            $notify[] = ['error', 'Please input less than your balance.'];
            return back()->withNotify($notify);
        }
        ...
        $user->update([
            'balance' => $prev_balance - $request->budget
        ]);
    ...
    public function remove($id)
    {
        ...
        $user->update([
            'balance' => $prev_balance + $ad->budget
        ]);
