...
class VisitorController extends Controller
{
    ...
    public function getAdvertise($pubId, $slug, $currentUrl)
    {
        ...
        $domain = DomainVerifcation::where('domain_name', $currentUrl)->whereStatus(1)->first();

        if ($domain == null || $domain->publisher_id != $publisherId) {
            return $this->defaultAd($slug, $adType->width, $adType->height, $setting->sitename);
        }
        ...
        if (isset($adType)) {
            ...
            $ads = CreateAd::where('ad_type_id', $adType->id)->whereStatus(1)->whereJsonContains('t_country', $query->country_name)->where
