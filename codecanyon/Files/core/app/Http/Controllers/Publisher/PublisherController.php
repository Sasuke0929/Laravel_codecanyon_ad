...
class PublisherController extends Controller
{
    ...
    {
        $page_title = 'Advertisement Referrals';
        $publisher = Auth::guard('publisher')->user();
        $advertisement = CreateAd::where('status', 1)->get();
        return view($this->activeTemplate.'publisher.referrals.referrals', compact('page_title', 'publisher', 'advertisement'));
    }

    public function referralsInsert()
    {
        $notify[] = ['success', 'This advertisement has been posted.'];
        return redirect()->route('publisher.referrals')->withNotify($notify);
    }
    ...
    public function domainVerify(Request $request)
    {
       ...
       $domainVerify->domain_name = $request->domain_name;
