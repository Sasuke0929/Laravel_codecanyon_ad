...
class GeneralSettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $request->merge(['domain_approval' => isset($request->domain) ? 1 : 0]);
        $request->merge(['registration' => isset($request->registration) ? 1 : 0]);

        $general_setting->update($request->only(['sitename','intervals','location_api', 'cur_text', 'cur_sym','address','phone','email', 'ev', 'en', 'sv', 'sn', 'registration', 'base_color', 'secondary_color','domain_approval']));
